...
- 개발자 홈페이지(매뉴얼 및 개발 이력): [https://note4all.tistory.com](https://note4all.tistory.com/category/%EC%86%8C%ED%94%84%ED%8A%B8%EC%9B%A8%EC%96%B4%20%EC%97%B0%EA%B5%AC%EC%86%8C/TIny%20PDF%20Editor)
...
- **가상 프린터...** (Windows·macOS): 다른 프로그램에서 인쇄한 결과를 PDF로 받아 이 프로그램에서 엽니다.
  - **Windows**: 프린터 목록에 `Tiny PDF Editor`를 넣습니다. (`Microsoft Print to PDF` 드라이버)
  - **macOS**: 인쇄 대화상자 **PDF** 메뉴에 `Tiny PDF Editor`를 넣습니다. 메뉴에서 「프린터 설치」를 누르면 프린터 목록에도 넣을 수 있으며, 이때는 관리자 암호가 필요할 수 있습니다.
...
- **1페이지 / 선택된 N개 페이지 / 전체페이지 OCR 실행**: 썸네일 우클릭. 스캔 페이지에 숨은 텍스트를 넣어 드래그 복사할 수 있습니다.
- **썸네일 크기**: `+` / `-` 버튼으로 조절
